...
OceanBase adapter 与 PostgreSQL 同形态，但走 MySQL 兼容协议接入 OceanBase（也支持轻量单机版 `oceanbase/seekdb`）。它是一个**生产导向的基线 adapter**，适合已经在用 OceanBase，或希望使用单机可嵌入 SQL 后端的团队。
...
- 通过 MySQL 兼容协议、基于 `pyobvector` 接入 OceanBase 的生产导向 SQL adapter 基线
- 复用 PostgreSQL adapter 的 lifecycle 语义
- 同样适用于轻量单机版 `oceanbase/seekdb`，便于资源受限环境
- 当前仅提供 `lexical` 搜索，向量索引能力暂未接通
...
- 配好 `MGP_OCEANBASE_DSN`（或 `MGP_OCEANBASE_*` 一组离散变量）后，`oceanbase` 也可以跑同一套 compliance
...
- `oceanbase` 是生产导向的 adapter 路径，可以在默认 CI 之外覆盖同一组 profile，也可对接 `oceanbase/seekdb`
